...
## MLC Doc Hub — mlcheatmap

Structured documentation lives in `.mlcai/`, maintained through the
`mlc-dochub` MCP server.

- **Project ID:** `mlcheatmap` — MLC Isometric Heatmap — SVG-Renderer
- **Source directory:** `/mnt/data2tb/mlcheatmap` (read source files with native Read —
...
- **Use the project id `mlcheatmap`.** Never invent `mlcheatmap-mac`, and
